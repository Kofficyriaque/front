...
const CareerAnalysis: React.FC = () => {
  ...
  async function verifTitre() {
    const data = await fetch("/metiers_informatique.txt");
    const lignes = (await data.text()).split(/\r?\n/);

    const titreValide = lignes
      .map(l => l.toLowerCase().trim())
      .includes(jobTitle.toLowerCase().trim());

    setFound(titreValide);

    if (titreValide) {
      setError("");
      nextStep();
    } else {
      setError("Le titre doit être bien formulé et du secteur de l'informatique");
    }
  }
